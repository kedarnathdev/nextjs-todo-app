      ...
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-5xl items-center justify-center">
        <div className="grid w-full overflow-hidden rounded-3xl border border-white/10 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-950 lg:grid-cols-2">
          <section className="hidden bg-gray-950 p-10 text-white dark:bg-zinc-900 lg:flex lg:flex-col lg:justify-between">
            <div><div className="mb-12 grid h-10 w-10 place-items-center rounded-xl bg-white text-sm font-black text-gray-950">F</div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/40">Flowlist</p><h1 className="mt-3 max-w-md text-4xl font-bold tracking-tight">Turn intentions into momentum.</h1><p className="mt-4 max-w-md text-sm leading-6 text-white/55">Create your focused workspace and start clearing your list.</p></div>
          ...
          <section className="bg-white p-7 dark:bg-zinc-950 sm:p-10">
            <div className="mb-8"><p className="eyebrow">Get started</p><h2 className="mt-2 text-2xl font-bold tracking-tight text-gray-950 dark:text-white">Create your Flowlist</h2><p className="mt-2 text-sm text-gray-500 dark:text-zinc-400">One simple place for everything you need to remember.</p></div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div><label className="mb-2 block text-sm font-semibold text-gray-700 dark:text-zinc-300" htmlFor="password">Password</label><input id="password" name="password" type="password" required minLength={8} autoComplete="new-password" placeholder="At least 8 characters" className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-[var(--accent)] focus:bg-white focus:ring-4 focus:ring-indigo-100 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100 dark:placeholder:text-zinc-600 dark:focus:bg-zinc-950 dark:focus:ring-indigo-500/10" /></div>
              <button type="submit" disabled={loading} className="w-full rounded-xl bg-gray-950 px-4 py-3.5 text-sm font-semibold text-white transition hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-white dark:text-gray-950 dark:hover:bg-zinc-200">{loading ? 'Creating account…' : 'Create account'}</button>
            </form>
            <p className="mt-7 text-center text-sm text-gray-500 dark:text-zinc-500">Already have an account? <Link href="/login" className="font-semibold text-gray-900 hover:text-[var(--accent)] dark:text-zinc-200">Sign in</Link></p>
          </section>
        </div>
      </div>
    </main>
